...
// Save complete_order data in a CSV file
app.get("/saveCompleteOrder", (req, res) => {
  db.all("SELECT * FROM complete_order", (err, rows) => {
    if (err) {
      console.error("Error retrieving complete order list:", err);
      res.status(500).json({ message: "Internal server error" });
    } else {
      const outputPath = path.join(__dirname, "complete_order.csv");
      const csvContent =
      ...
      rows.forEach((row) => {
        ...
        const csvRow = `${customer_name},${item_name},${date_of_order},${quantity},${date_of_delivery}\n`;
        csvContent += csvRow;
      });

      fs.writeFile(outputPath, csvContent, (writeErr) => {
        if (writeErr) {
          console.error("Error writing output file:", writeErr);
          res.status(500).json({ message: "Internal server error" });
        } else {
          console.log("Complete order data saved successfully");
          res.download(outputPath, "complete_order.csv", (downloadErr) => {
            if (downloadErr) {
